...
class Challenges extends React.Component {
  ...
  telephoneValidator(toCheck) {
    ...
    if (toCheck) {
      ...
      if (result)
        restxt = "Yep, you've got yourself a valid number";
      else {
  ...
  testPalindrome (toCheck) {
    ...
    if (toCheck) {
      ...
      if (result)
        restxt = "Yep, you've got yourself a palindrome";
      else {
    ...
    return(
      <div class="challenge-div">
        <h1 class="description">
          { challenge_text[challenge_type].desc }
        </h1>
        <p class="desc-text">
          { challenge_text[challenge_type].text }
        </p>
          ...
          <input type="button" 
                 value="Submit" 
                 class="submit-button" onClick={this.runAlgorithm}/>
...
function ToolTip(contents) {
  return (
    <span class="tooltiptext">{contents}</span>
  )
}
...
class MoneyMap extends React.Component {
  ...
  render () {
    ...
    const b0 = ((type=="input" || rawValue > 0) && <label class="currency-label">{label}</label>)
    const b1 =  (type=="input" &&
                 <div class="tooltip">
                 <button type="button" onClick={ e => this.changeValue("inc") }>+</button>
                 {ToolTip("Increment this value")}
                 </div>
                )
    const b2 = ((type=="input" || rawValue > 0) && <label class="currency">{value}</label>)
    const b3 =  (type=="input" &&  
                 <div class="tooltip">
                 <button type="button" onClick={ e => this.changeValue("dec") }>-</button>
  ...
  render () {
    return(      
        <select name="dropdown"
                ...
                onChange={ e => this.setParentState(e.target.value) }>

          <option selected="true" disabled value=''>
            Preset Inputs
          </option>
          {
            REGISTER_PRESETS.map((item, i) => 
...
class RegisterInputs extends React.Component {
  ...
  render () {
    return(
      <div class="register-display-panel" 
           id="register-inputs">
        <div class="input-field">
          <label class="field-label">Item Cost</label>
        ...
        <div class="break"></div>
        <div class="input-field">
          <label class="field-label">Customer Payment</label>
        ...
        <div class="break"></div>
        <div class="cash-and-change">
          <label id="cash-register-buttons-title">Cash In Register: </label>
          {
            MONEY.map ((item, i) => 
              <MoneyMap
                label={item[0]}
        ...
        <div class="button-bar">
          <div class="tooltip div-width">
            <PresetInputs
               state={this.props.state}
               changeState={this.props.changeState} />
            {
            ToolTip("Preset values for testing")
            }
          </div>
          <div class="tooltip div-width">
            <input type="button" 
...
class RegisterOutputs extends React.Component {
  ...
  render () {
    ...
    const b0 = ((status)    && <> <label>Result Status: </label>
                                  <label>{this.props.state.status}</label>
                                  <div class="break"></div>
                               </>)
    
    // Money can be empty if no change is required
    const b1 = ((!no_money) && <label>Change due:</label>)

    return(    
      <div  class="register-display-panel tooltip" 
            id="register-outputs">
        { b0 } 
        <div class="cash-and-change">
        { b1 }
...
class CashRegisterChallenges extends React.Component {
  constructor(props) {
    ...
    this.state = {
      price: 0,
      payment: 0,
      cid: { penny: 0, nickel: 0, dime: 0,  quarter: 0,
  ...
  reset() {
    const cid   ={ penny: 0, nickel: 0, dime: 0,  quarter: 0, one: 0, five: 0, ten: 0, twenty: 0, hundred: 0 }
  ...
  // Main function to run the FCC challenge code
  ...
  render () {
    return(
      <div id="cash-register-details" class="challenge-div">
        <h1 class="description">
          { challenge_text[challenge_type].desc }
        </h1>
        <p class="desc-text">
...
function algorithmSelected() {
  ...
  freshRender=true;
  ReactDOM.render(<App />, document.getElementById("activity"));
}
